...
fn bench_unix_mt_with_offline_base(
    c: &mut Criterion,
    // secs: u64,
    nparties: usize,
    min_e: usize,
    ...
    let plot_config = PlotConfiguration::default().summary_scale(AxisScale::Logarithmic);
    let mut group = c.benchmark_group(format!("time_unix_stream_mt_compare_{}_with_offline", name));
    group.plot_config(plot_config);
    // group.measurement_time(Duration::from_secs(secs));
    for e in min_e..=max_e {
        if e >= 14 {
...
fn bench_unix_mt_with_offline_short(c: &mut Criterion) {
    let nparties = 5;
    let min_e = 3;
    let max_e = 5;
    bench_unix_mt_with_offline_base(c, nparties, min_e, max_e, "short");
}

fn bench_unix_mt_with_offline_middle(c: &mut Criterion) {
    let nparties = 5;
    let min_e = 3;
    let max_e = 12;
    bench_unix_mt_with_offline_base(c, nparties, min_e, max_e, "middle");
}

fn bench_unix_mt_with_offline_long(c: &mut Criterion) {
    let nparties = 5;
    let min_e = 3;
    let max_e = 20;
    bench_unix_mt_with_offline_base(c, nparties, min_e, max_e, "long");
}
...
criterion_group!(
    ...
    targets = bench_unix_mt_short, bench_unix_mt_middle, bench_unix_mt_long,
        bench_unix_mt_with_offline_short, bench_unix_mt_with_offline_middle, bench_unix_mt_with_offline_long,
        bench_tcp_mt_short, bench_tcp_mt_middle, bench_tcp_mt_long
);
